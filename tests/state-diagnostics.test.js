...
test('the rendered stat set did not silently drop fields (renderer contract)', () => {
  ...
  for (const tool of buildState().tools) {
    for (const key of ['source', 'label', 'plan', 'haveLimits', 'limits', 'modelLimits', 'accountLimits', 'projection', 'activity', 'dataAt', 'limitsDiagnostic', 'freshness']) {
    ...
    assert.equal(tool.accountLimits.scope, 'account-wide');
